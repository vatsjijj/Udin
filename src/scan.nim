...
proc keyword(word: string): (Token, string) =
  ...
  of "enableMemo": return (Token.enablememo, tok)
...
proc transpile*(tbl: seq[(Token, string)] = tokenTable): string =
  var output: string = "# Generated by Udin 0.3.3\n\n"
  if isEnum:
    output = output & "# Modules imported by Udin #\n"
  ...
  for i in 0..len(tbl) - 1:
    ...
    of Token.enablememo: output = output & "from functools import cache"
      ...
      if i != 0:
        ...
        if lookahead(tbl, Token.atom, i):
          ...
          if execCmd("find " & tbl[i + 1][1] & ".udin &> /dev/null") != 0 and not lookahead(tbl, Token.star, i):
            ...
            if toTranspile.contains(tbl[i + 1][1] & "_udin"):
    ...
    of Token.atom:
      if i != 0:
        if lookback(tbl, Token.imprt, i) or lookback(tbl, Token.frm, i):
          if execCmd("find " & tbl[i][1] & ".udin &> /dev/null") == 0:
            continue
          else:
      ...
      if types.contains(tbl[i - 1][0]):
        ...
        if types.contains(tbl[i + 1][0]):
          if lookahead(tbl, Token.str, i):
            ...
            rhs = " " & tbl[i + 1][1] & "\""
          output = output & "case " & lhs & ": return " & rhs
        else:
